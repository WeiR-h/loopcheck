...
A 127-second captioned evidence walkthrough is available in the release artifacts; it is an edited combination of real browser clips and UI stills, not the final continuous desktop recording. Source-archive startup passed with the existing Python dependencies; a fresh Docker build remains unverified because the daemon was unavailable.

Public repository, hosted judging URL, public video URL and Devpost submission are **not yet verified as published**. AgentCore is not deployed; the genuine AWS contribution is Strands Agents SDK. [Release checklist](docs/参赛检查清单.md)
