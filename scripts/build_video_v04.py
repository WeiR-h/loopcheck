...
work = ROOT / 'data/video-v04'
dist = ROOT / 'dist'
dist.mkdir(exist_ok=True)
ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()
scenes = json.loads((ROOT/'docs/video/scenes-v04.json').read_text(encoding='utf-8'))
...
output=dist/'LoopCheck-demo-en-v0.4.mp4'
...
(dist/'LoopCheck-demo-en-v0.4.srt').write_text('\n'.join(all_srt),encoding='utf-8')
assert total < 300, total
subprocess.run([ffmpeg,'-hide_banner','-loglevel','error','-i',str(output),'-f','null','-'],check=True)
metadata={'duration_seconds':round(total,2),'format':'1920x1080 H.264/AAC','source':'Actual v0.4 UI screenshots plus recorded Chromium budget flows; silent audio track; English captions','disclosure':'Edited evidence walkthrough, not a continuous desktop recording; real browser clips hold their last frame during explanation; faults deliberately injected','bytes':output.stat().st_size}
(ROOT/'docs/evidence/v04-video-check.json').write_text(json.dumps(metadata,indent=2),encoding='utf-8')
