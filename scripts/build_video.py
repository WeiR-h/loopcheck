"""Assemble verified screenshots with local synthetic narration and English captions."""
from pathlib import Path
import json, re, subprocess, textwrap, wave
import imageio_ffmpeg

ROOT = Path(__file__).resolve().parents[1]
work = ROOT / 'data/video'
dist = ROOT / 'dist'
dist.mkdir(exist_ok=True)
ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()
scenes = json.loads((ROOT/'docs/video/scenes.json').read_text(encoding='utf-8'))
total = 0
all_srt = []
...
for i, scene in enumerate(scenes):
    ...
    command=[ffmpeg,'-hide_banner','-loglevel','error','-y','-loop','1','-framerate','12','-i',
        str(ROOT/'docs/audit-v1'/scene['image']),'-i',str(work/f'voice-{i}.wav'),
        '-vf',f'scale=1920:900:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:60:color=0x122319,subtitles=scene-{i}.ass',
...
output=dist/'LoopCheck-demo-en-v0.3.mp4'
...
(dist/'LoopCheck-demo-en-v0.3.srt').write_text('\n'.join(all_srt),encoding='utf-8')
assert total < 300, total
subprocess.run([ffmpeg,'-hide_banner','-loglevel','error','-i',str(output),'-f','null','-'],check=True)
metadata={'duration_seconds':round(total,2),'format':'1920x1080 H.264/AAC','source':'Actual UI screenshots; synthetic Microsoft Zira narration; English captions','disclosure':'Narrated screenshot walkthrough, not a continuous real-time recording','bytes':output.stat().st_size}
(ROOT/'docs/evidence/video-check.json').write_text(json.dumps(metadata,indent=2),encoding='utf-8')
