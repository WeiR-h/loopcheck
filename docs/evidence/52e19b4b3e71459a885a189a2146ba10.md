...
- Run: 52e19b4b3e71459a885a189a2146ba10
- Status: adopted
- Started (UTC): 2026-09-09T09:36:02.711801+00:00
- Model: qwen3.7-flash
- Trigger: guard_repair
- Elapsed: 37.11 seconds (machine and API time included)
- Provider-reported estimate: CNY 0.003104
- Reserved without returned usage: CNY 0.000000
- Requests: 5

## Reported problem

后续改版出现回归，请恢复以下要求并保留其他功能：删除同名项，保留另一条记录；不同排列下只删除选中项；删除同名任务时只删除选中项
...
| Delete only the selected task with duplicate titles | FAIL | PASS | 保存的 6 个操作与断言已执行 |

## Reusable requirement

```json
{
  "title": "Delete only the selected task with duplicate titles",
  "steps": [
    {
      "action": "add",
      "value": "Task A",
